...
UINT24_HALF :: u32(0x0080_0000)

uint24_inc :: proc(value: ^UInt24) -> UInt24 {
    old := value^
    value^ = UInt24((u32(value^) + 1) & UINT24_MASK)
    return old
}

uint24_next :: proc(value: UInt24) -> UInt24 {
    return UInt24((u32(value) + 1) & UINT24_MASK)
}

uint24_add :: proc(value: UInt24, offset: u32) -> UInt24 {
    return UInt24((u32(value) + offset) & UINT24_MASK)
}

uint24_forward_distance :: proc(first, last: UInt24) -> u32 {
    return (u32(last) - u32(first)) & UINT24_MASK
}

uint24_before :: proc(first, last: UInt24) -> bool {
    distance := uint24_forward_distance(first, last)
    return distance != 0 && distance < UINT24_HALF
}
