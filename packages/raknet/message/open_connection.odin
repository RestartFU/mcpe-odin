...
unmarshal_open_connection_request_1 :: proc(data: []u8) -> (
    ...
    // Pinned go-raknet skips, but does not compare, UNCONNECTED_MAGIC.
...
unmarshal_open_connection_request_2 :: proc(
    ...
    if len(data) < len(UNCONNECTED_MAGIC) {
        err = mcpe_runtime.make_error(.Unexpected_EOF, "wire.unmarshal_open_connection_request_2")
        return
    }
    r := wire.reader(data[len(UNCONNECTED_MAGIC):])
