...
datagram_window_seen :: proc(window: ^Datagram_Window, index: UInt24) -> bool {
    if uint24_before(index, window.lowest) {
        return true
    }
...
datagram_window_missing :: proc(
    ...
    distance := int(uint24_forward_distance(window.lowest, window.highest))
    for offset := distance - 1; offset >= 0; offset -= 1 {
        index := uint24_add(window.lowest, u32(offset))
...
datagram_window_size :: proc(window: ^Datagram_Window) -> UInt24 {
    return UInt24(uint24_forward_distance(window.lowest, window.highest))
}
