...
MAX_RESEND_PACKETS :: 4096
MAX_RESEND_BYTES   :: 64 * 1024 * 1024
...
resend_map_add :: proc(
    ...
    if _, exists := value.unacknowledged[index]; exists {
        return false
    }
    if len(value.unacknowledged) >= MAX_RESEND_PACKETS ||
       value.bytes + len(packet.content) > MAX_RESEND_BYTES {
        return false
    }
    ...
    value.bytes += len(packet.content)
    return true
}
...
resend_map_remove :: proc(
    ...
    value.bytes -= len(record.packet.content)
