...
UPSTREAM_DIR="$ROOT_DIR/.cache/upstream/gophertunnel"
LOCKED_COMMIT=$(awk '
    $0 == "[gophertunnel]" { found=1; next }
...
if [[ ! -d "$UPSTREAM_DIR/.git" ]]; then
    ...
    git clone --filter=blob:none \
        https://github.com/Sandertv/gophertunnel.git \
        "$UPSTREAM_DIR"
fi
...
ORACLE_PACKAGE="$UPSTREAM_DIR/cmd/mcpe-odin-codec-oracle"
mkdir -p "$ORACLE_PACKAGE"
cp "$ROOT_DIR/tests/differential/go-gophertunnel-codec-oracle/main.go" \
...
(
    cd "$UPSTREAM_DIR"
    go build -trimpath -o "$WORK_DIR/go-oracle" \
        ./cmd/mcpe-odin-codec-oracle
)
"$ROOT_DIR/tools/odinw" build gophertunnel-fixtures \
...
printf 'gophertunnel codec fixtures match %s\n' "$LOCKED_COMMIT"
